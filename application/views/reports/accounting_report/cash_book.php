...
function loadData(pdf=""){
	...
	var postData = {from_date:from_date,to_date:to_date,group_code:"'CS'",pdf:pdf};
	if(valid){
		if(pdf == ""){
			$.ajax({
				url: base_url + controller + '/getCashBookData',
				data: postData,
				type: "POST",
				dataType:'json',
			...
			var url = base_url + controller + '/getCashBookData/' + encodeURIComponent(window.btoa(JSON.stringify(postData)));
			window.open(url);
		}
