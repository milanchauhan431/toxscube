...
function resItemDetail(response = ""){
    if(response != ""){
        ...
        $("#itemForm #hsn_code").val(partyDetail.hsn_code);$("#itemForm #hsn_code").select2();
        $("#itemForm #gst_per").val(parseFloat(partyDetail.gst_per));$("#itemForm #gst_per").select2();
    }else{
        $("#itemForm #item_code").val("");
...
function resGstIncome(data,formId){
    if(data.status==1){
        $('#'+formId)[0].reset();
